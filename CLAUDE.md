...
`main.go` (`package main`, repo root) is a thin shell: it just calls `app.Run`.
All logic lives in `internal/app` (`package app`), so it can be tested from an external `app_test` package.
`go build` at the repo root still produces the `gh` extension binary.
Every tested identifier is exported from `internal/app`.

The program has two entry paths in `internal/app/run.go` (`app.Run`).
...
- **Org path** (`internal/app/list_alerts.go`, `ListAlertsForOrg`): calls `orgs/{org}/dependabot/alerts`.
  It returns alerts across all repos in the org directly, including repository info per alert.
- **User path** (`internal/app/list_alerts.go`, `ListAlertsForUser`): no org-wide endpoint exists for user repos.
...
Both paths only request `state=open` alerts (`OpenAlertsURL` in `internal/app/list_alerts.go`).

**Pagination** (`internal/app/pagination.go`, `internal/app/next_path.go`): `FetchAllPages[T]` is a generic helper.
...
**Rate limiting** (`internal/app/rate_limiter.go`): a package-level `limiter` throttles outgoing requests.
It's a `golang.org/x/time/rate.Limiter`, built via `rate.NewLimiter(5, 1)`.
That's 5 requests per second, with a burst size of one.
`FetchPage` (`internal/app/pagination.go`) calls `Limiter.Wait(ctx)` before every single HTTP request.
That blocks until the limiter admits the request.
`client` and `limiter` are bundled into one `*GithubClient` (`internal/app/pagination.go`).
...
It comes down from `app.Run`, itself injectable via `newClient` (production wiring is `app.NewDefaultGithubClient`).
So tests can exercise the whole call chain against a fake HTTP server.
That covers `internal/app/run_test.go`, `internal/app/list_alerts_test.go`, and `internal/app/pagination_test.go`.
The test files are `package app_test` (black-box); shared helpers live in `internal/app/restclient_test.go`.
Tests build one via `newTestGithubClient()` and `noWaitLimiter()` (`internal/app/restclient_test.go`).
...
**Response shrinking** (`internal/app/small_dependabot_alert.go`): trims the `github.DependabotAlert` struct.
...
**Error handling** (`internal/app/fatal.go`): errors are wrapped with `github.com/cockroachdb/errors` throughout.
